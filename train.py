...
from utils.loss import SoftLoULoss, DICE, F_Loss
from utils.Focal_Loss import BCEFocalLoss,IOUloss
from utils.lr_scheduler import *
from utils.metrics import SegmentationMetricTPFNFP
from utils.clip import clip_gradient
import openpyxl
...
def parse_args():
    ...
    parser.add_argument('--dataset', type=str, default='ISTS', help='choose datasets')
    ...
    parser.add_argument('--learning-rate', type=float, default=0.00008, help='learning rate')
    ...
    parser.add_argument('--save-iter-step', type=int, default=1000, help='save model per step iters')

    #
    # Net parameters
    #
    parser.add_argument('--net-name', type=str, default='agpcnet_1',
                        help='net name: fcn')
...
class Trainer(object):
    def __init__(self, args):
        ...
        ## dataset
        if args.dataset == 'ISTS':
            trainset = ISTSDataset(mode='train')
            valset = ISTSDataset(mode='test')
        ...
        self.scheduler = LR_Scheduler_Head(args.lr_scheduler, args.learning_rate,
                                      args.epochs, len(self.train_data_loader), lr_step=100)

        param_pretrain=[]
        param_train=[]
        for name,params in self.net.named_parameters():
            if 'backbone' in name:
                param_pretrain.append(params)
            else:
                param_train.append(params)
        ## optimizer
        ...
        self.optimizer = torch.optim.SGD(param_train, lr=args.learning_rate, momentum=0.9, weight_decay=1e-4)
        self.optimizer.add_param_group({"params":param_pretrain,"lr":args.learning_rate*0.1})
    ...
    def training(self, epoch):
        ...
        for i, (data, labels) in enumerate(tbar):
            ...
            loss = fl(output,labels)+iou(output,labels)
            ...
            #ws.cell(row=(self.iter_num + 1), column=1).value = np.mean(losses)
            #wb.save('loss.xlsx')
    ...
    def validation(self):
        ...
        for i, (data, labels) in enumerate(self.val_data_loader):
            ...
            #loss = self.criterion(output, labels)
            loss = iou(output, labels) + fl(output, labels)
            #loss = fl(output,labels)
        ...
        #if (epoch) % 2 == 0:
         #   torch.save(self.net, osp.join(self.save_pkl, 'epoch-' +'%d' % epoch))
